...
class TranslateAttrNAttribute implements Attribute
{
    ...
    public function visit(DOMAttr $att, Compiler $context)
    {
        ...
        foreach ($expressions as $expression) {
            ...
            $transParams = trim($attrExpr[1], "\n\t\r []");
            $transParams = ParserHelper::staticSplitExpression($transParams, ",", 2);

            $parts[$attrExpr[0]] = "['" . addcslashes($attNode->value, "'") . "'|transchoice($transParams[0]" . (isset($transParams[1]) ? ("," . $transParams[1]) : '') . "" . ($transParams[2] ? ",$transParams[2]" : "") . ")]";
